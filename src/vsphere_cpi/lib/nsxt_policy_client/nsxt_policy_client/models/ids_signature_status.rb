...
module NSXT
  ...
  class IdsSignatureStatus
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'download_status')
        self.download_status = attributes[:'download_status']
      end

      if attributes.has_key?(:'signature_status')
        self.signature_status = attributes[:'signature_status']
      end

      if attributes.has_key?(:'version_id')
    ...
    def valid?
      ...
      download_status_validator = EnumAttributeValidator.new('String', ['READY', 'PENDING', 'ERROR', 'DISABLED'])
      return false unless download_status_validator.valid?(@download_status)
      signature_status_validator = EnumAttributeValidator.new('String', ['AVAILABLE', 'UNAVAILABLE'])
    ...
    # @param [Object] download_status Object to be assigned
    def download_status=(download_status)
      validator = EnumAttributeValidator.new('String', ['READY', 'PENDING', 'ERROR', 'DISABLED'])
      unless validator.valid?(download_status)
        fail ArgumentError, 'invalid value for "download_status", must be one of #{validator.allowable_values}.'
      end
      @download_status = download_status
    end
    ...
    def hash
      [_self, _links, _schema, _revision, _system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, path, parent_path, unique_id, relative_path, children, overridden, marked_for_delete, download_status, signature_status, version_id].hash
    end
