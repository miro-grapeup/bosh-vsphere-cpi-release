...
module NSXT
  ...
  class IdsRule
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'ids_profiles')
        if (value = attributes[:'ids_profiles']).is_a?(Array)
          self.ids_profiles = value
        end
      end
    end
    ...
    def valid?
      ...
      action_validator = EnumAttributeValidator.new('String', ['DETECT', 'DETECT_PREVENT'])
    ...
    def action=(action)
      validator = EnumAttributeValidator.new('String', ['DETECT', 'DETECT_PREVENT'])
    ...
    def hash
      [_self, _links, _schema, _revision, _system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, path, parent_path, unique_id, relative_path, children, overridden, marked_for_delete, disabled, direction, ip_protocol, notes, logged, profiles, rule_id, is_default, tag, source_groups, destination_groups, services, scope, service_entries, destinations_excluded, sequence_number, sources_excluded, action, ids_profiles].hash
    end
