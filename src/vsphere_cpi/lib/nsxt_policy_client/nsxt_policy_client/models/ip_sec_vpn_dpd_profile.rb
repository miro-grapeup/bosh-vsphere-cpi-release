...
module NSXT
  ...
  class IPSecVpnDpdProfile
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'dpd_probe_mode')
        self.dpd_probe_mode = attributes[:'dpd_probe_mode']
      else
        self.dpd_probe_mode = 'PERIODIC'
      end
    ...
    def list_invalid_properties
      ...
      if !@retry_count.nil? && @retry_count > 100
        invalid_properties.push('invalid value for "retry_count", must be smaller than or equal to 100.')
      end

      if !@retry_count.nil? && @retry_count < 1
        invalid_properties.push('invalid value for "retry_count", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@retry_count.nil? && @retry_count > 100
      return false if !@retry_count.nil? && @retry_count < 1
      dpd_probe_mode_validator = EnumAttributeValidator.new('String', ['PERIODIC', 'ON_DEMAND'])
      return false unless dpd_probe_mode_validator.valid?(@dpd_probe_mode)
      true
    end
    ...
    def retry_count=(retry_count)
      if !retry_count.nil? && retry_count > 100
        fail ArgumentError, 'invalid value for "retry_count", must be smaller than or equal to 100.'
      end

      if !retry_count.nil? && retry_count < 1
        fail ArgumentError, 'invalid value for "retry_count", must be greater than or equal to 1.'
      end
    ...
    # @param [Object] dpd_probe_mode Object to be assigned
    def dpd_probe_mode=(dpd_probe_mode)
      validator = EnumAttributeValidator.new('String', ['PERIODIC', 'ON_DEMAND'])
      unless validator.valid?(dpd_probe_mode)
        fail ArgumentError, 'invalid value for "dpd_probe_mode", must be one of #{validator.allowable_values}.'
      end
      @dpd_probe_mode = dpd_probe_mode
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          enabled == o.enabled &&
          dpd_probe_mode == o.dpd_probe_mode &&
    ...
    def hash
      [_self, _links, _schema, _revision, _system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, path, parent_path, unique_id, relative_path, children, overridden, marked_for_delete, retry_count, enabled, dpd_probe_mode, dpd_probe_interval].hash
    end
