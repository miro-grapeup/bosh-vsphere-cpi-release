...
module NSXT
  ...
  class DhcpV6StaticBindingConfig
    ...
    def initialize(attributes = {})
      ...
      if attributes.has_key?(:'sntp_servers')
        if (value = attributes[:'sntp_servers']).is_a?(Array)
          self.sntp_servers = value
        end
      end

      if attributes.has_key?(:'preferred_time')
        self.preferred_time = attributes[:'preferred_time']
      end
    ...
    def list_invalid_properties
      ...
      if !@preferred_time.nil? && @preferred_time > 4294967295
        invalid_properties.push('invalid value for "preferred_time", must be smaller than or equal to 4294967295.')
      end

      if !@preferred_time.nil? && @preferred_time < 48
        invalid_properties.push('invalid value for "preferred_time", must be greater than or equal to 48.')
      end
    ...
    def valid?
      ...
      resource_type_validator = EnumAttributeValidator.new('String', ['DhcpV4StaticBindingConfig', 'DhcpV6StaticBindingConfig'])
      return false unless resource_type_validator.valid?(@resource_type)
      return false if !@preferred_time.nil? && @preferred_time > 4294967295
      return false if !@preferred_time.nil? && @preferred_time < 48
    ...
    def resource_type=(resource_type)
      validator = EnumAttributeValidator.new('String', ['DhcpV4StaticBindingConfig', 'DhcpV6StaticBindingConfig'])
    ...
    # Custom attribute writer method with validation
    # @param [Object] preferred_time Value to be assigned
    def preferred_time=(preferred_time)
      if !preferred_time.nil? && preferred_time > 4294967295
        fail ArgumentError, 'invalid value for "preferred_time", must be smaller than or equal to 4294967295.'
      end

      if !preferred_time.nil? && preferred_time < 48
        fail ArgumentError, 'invalid value for "preferred_time", must be greater than or equal to 48.'
      end
    ...
    def hash
      [_self, _links, _schema, _revision, _system_owned, display_name, description, tags, _create_user, _protection, _create_time, _last_modified_time, _last_modified_user, id, resource_type, path, parent_path, unique_id, relative_path, children, overridden, marked_for_delete, sntp_servers, preferred_time, lease_time, mac_address, ip_addresses, dns_nameservers, domain_names].hash
    end
