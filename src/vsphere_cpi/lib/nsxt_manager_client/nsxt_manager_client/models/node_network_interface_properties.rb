...
module NSXT
  ...
  class NodeNetworkInterfaceProperties
    ...
    def list_invalid_properties
      ...
      if !@vlan.nil? && @vlan > 4094
        invalid_properties.push('invalid value for "vlan", must be smaller than or equal to 4094.')
      end

      if !@vlan.nil? && @vlan < 1
        invalid_properties.push('invalid value for "vlan", must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@vlan.nil? && @vlan > 4094
      return false if !@vlan.nil? && @vlan < 1
      bond_mode_validator = EnumAttributeValidator.new('String', ['ACTIVE_BACKUP', '802_3AD', 'ROUND_ROBIN', 'BROADCAST', 'XOR', 'TLB', 'ALB'])
      ...
      plane_validator = EnumAttributeValidator.new('String', ['mgmt', 'debug', 'none'])
    ...
    def vlan=(vlan)
      if !vlan.nil? && vlan > 4094
        fail ArgumentError, 'invalid value for "vlan", must be smaller than or equal to 4094.'
      end

      if !vlan.nil? && vlan < 1
        fail ArgumentError, 'invalid value for "vlan", must be greater than or equal to 1.'
      end
    ...
    def bond_mode=(bond_mode)
      validator = EnumAttributeValidator.new('String', ['ACTIVE_BACKUP', '802_3AD', 'ROUND_ROBIN', 'BROADCAST', 'XOR', 'TLB', 'ALB'])
    ...
    def plane=(plane)
      validator = EnumAttributeValidator.new('String', ['mgmt', 'debug', 'none'])
